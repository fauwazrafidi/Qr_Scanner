...
using Xamarin_Scanner_example;
using static Android.Widget.RadioGroup;

namespace com.companyname.xamarin_scanner_example
{
    [Activity(Label = "Lingkail QRScanner", MainLauncher = true)]
    public class MainActivity : AppCompatActivity
    {
        MySampleBroadcastReceiver receiver;
        public EditText edittext;
        Button startScan, stopScan, btnScanQRCode;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            btnScanQRCode = FindViewById<Button>(Resource.Id.btn_scanQRCode);
    ...
    protected override void OnResume()
        ...
        public class MySampleBroadcastReceiver : BroadcastReceiver
        {
            public override void OnReceive(Context context, Intent intent)
            {
                ...
                string scannedData = intent.GetStringExtra(ScanManager.BarcodeStringTag);

                //Toast.MakeText(Application.Context, scannedData, ToastLength.Short).Show();

                if (scannedData != null)
                {
                    Intent displayIntent;
                    if (scannedData.Contains("/RawMaterial/"))
                    {
                        displayIntent = new Intent(context, typeof(DisplayDataActivity));
                        displayIntent.PutExtra("ScanResultUrl", scannedData);
                        displayIntent.SetFlags(ActivityFlags.NewTask);
                        context.StartActivity(displayIntent);
                    }
                    else if (scannedData.Contains("/resultitem/"))
                    {
                        displayIntent = new Intent(context, typeof(FinishedGoodsActivity));
                        displayIntent.PutExtra("ScanResultUrl", scannedData);
                        displayIntent.SetFlags(ActivityFlags.NewTask);
                        context.StartActivity(displayIntent);
                    }
                    ...
                    {
                        ShowAlertDialog("Alert", "Invalid QR Code", context);
                    }
                }
